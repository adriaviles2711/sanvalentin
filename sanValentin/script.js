...
function buttonRunsAway() {
    ...
    function runAway(e) {
        ...
        if (dist < 180) {
            ...
            const nx = Math.max(20, Math.min(window.innerWidth - 100, bx - Math.cos(angle) * 120));
            const ny = Math.max(20, Math.min(window.innerHeight - 60, by - Math.sin(angle) * 120));
...
function mathChallenge() {
    ...
    minigameTitle.textContent = `Resuelve para decir NO: ${a} ${op} ${b} = ?`;
    minigameContent.innerHTML = `
        <input type="text" id="mathInput" placeholder="Respuesta">
        <button onclick="checkMath(${answer})">Enviar</button>
    `;
...
window.checkMath = function (answer) {
    ...
    if (parseInt(input.value) === answer) {
        ...
        input.value = '';
        input.placeholder = 'MAL, intenta otra vez';
    }
};
...
function riddleChallenge() {
    isMinigameActive = true;
    const riddles = [
        { q: "¿Qué tiene cabeza pero no cerebro? (Pista: tú con M mayúscula y el resto minuscula)", a: "Marla" },
        { q: "¿Qué se moja al secar? (Pista: te secas con ella, todo minuscula, es que si no te lo digo no aciertes, palurda)", a: "toalla" },
        { q: "Tiene agujas pero no cose (Pista: tiktak, todo minuscula)", a: "reloj" },
        { q: "La persona que más quieres en este mundo (Pista: yo con A mayúscula y acabado en dri)", a: "Adri" }
    ];
...
function escapeRoom() {
    ...
    const hints = [
        `La primera cifra es ${Math.floor(code / 1000)}`,
        `La segunda cifra es ${Math.floor((code % 1000) / 100)}`,
        `La tercera cifra es ${Math.floor((code % 100) / 10)}`,
        `La última cifra es ${code % 10}`
    ];

    minigameTitle.textContent = "ESCAPE ROOM: Descifra el código";
    minigameContent.innerHTML = `
        <p style="font-size:0.8rem">${hints.join(' | ')}</p>
        <input type="text" id="codeInput" placeholder="Código de 4 dígitos" maxlength="4">
...
const chaosEvents = [
    ...
    mathRace,
    simonSaysHard,
    () => { invertPage(); shakeCard(); },
    () => { rotatePage(); },
    () => { tiltPage(); addYesButtons(); },
    () => { blurPage(); growYes(); },
    () => { huePage(); addYesButtons(); growYes(); }
];
